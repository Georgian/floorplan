...
public class OpacityTenAction extends SelectionAction {
	...
	public OpacityTenAction(final IWorkbenchPart part,  final CanvasModel model) {
		super(part);
		this.model = model;
	...
	public void run() {
		...
		final Request opacityReq = new Request("opacity"); //$NON-NLS-1$
		final HashMap<String, String> reqData = new HashMap<String, String>();
		reqData.put("newOpacity", getTen()); //$NON-NLS-1$
		opacityReq.setExtendedData(reqData);
		...
		if (selection == 0) {
			for (final ChildModel childModel : model.getChildren())
				childModel.setOpacity( Integer.parseInt(getTen()) );
		}
	}
